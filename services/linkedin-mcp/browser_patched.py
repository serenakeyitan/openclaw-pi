...
async def get_or_create_browser(
    ...
    # Inject li_at cookie from LINKEDIN_COOKIE env var if set
    li_at_cookie = os.environ.get("LINKEDIN_COOKIE")
    if li_at_cookie:
        logger.info("Injecting li_at cookie from LINKEDIN_COOKIE env var")
        # Clear any stale LinkedIn cookies first to avoid redirect loops
        try:
            await browser.context.clear_cookies(domain=".linkedin.com")
            logger.info("Cleared stale LinkedIn cookies from browser context")
        except Exception as e:
            logger.debug("Could not clear cookies (may be empty): %s", e)
    ...
    await browser.page.goto("https://www.linkedin.com/feed/", timeout=60000)
